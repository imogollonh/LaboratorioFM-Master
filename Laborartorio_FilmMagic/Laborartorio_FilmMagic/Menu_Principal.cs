...
namespace Laborartorio_FilmMagic
{
    public partial class Menu_Principal : Form
    {
        ...
        bool ventanaConceptoa = false;
        Frm_asignacionConcepto conceptoa = new Frm_asignacionConcepto();
        private void conceptosToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Form frmC = Application.OpenForms.Cast<Form>().FirstOrDefault(x => x is Frm_asignacionConcepto);
            if (ventanaConceptoa == false || frmC == null)
            {
                if (frmC == null)
                {
                    conceptoa = new Frm_asignacionConcepto();
                }

                conceptoa.MdiParent = this;
                conceptoa.Show();
                Application.DoEvents();
                ventanaConceptoa = true;

            }
            else
            {
                conceptoa.WindowState = System.Windows.Forms.FormWindowState.Normal;
            }
        }
    }
}
